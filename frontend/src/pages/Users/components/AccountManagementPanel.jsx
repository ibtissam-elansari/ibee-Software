...
const AccountManagementPanel = () => {
  ...
  const submitCreate = (data) => createMutation.mutate(data,                   { onSuccess: closeModal });
  const submitEdit   = (data) => updateMutation.mutate({ id: targetUser.id, data }, { onSuccess: closeModal });
    ...
    <>
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 flex flex-col gap-5">
        ...
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">Gérez les accès et les rôles de l'équipe.</p>
        </div>
              ...
              {isLoading ? (
                ...
                filtered.map(user => (
                  <tr key={user.id}
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="min-w-0">
                          <p className="text-sm font-semibold text-gray-900 truncate">
                            {/* show display name if available, fallback to email prefix */}
                    ...
                    {/* Actions — edit only (matches Figma: only pencil icon) */}
                    <td className="px-3 py-3.5">
                      {canManage(actorRole, user.role) ? (
                        <button
